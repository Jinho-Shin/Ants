...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if isinstance(insect,QueenAnt) and insect.is_impostor == False:
            return
        if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        scope = self.place
        distance = 0
        while scope is not beehive and distance <= self.max_range:
            if self.min_range > distance or not scope.bees:
                distance += 1
                scope = scope.entrance
            else:
                return random_or_none(scope.bees)
    ...
    def action(self, colony):
        ...
        def double_damage(ant):
            if ant not in QueenAnt.doubled_ants:
                ant.damage *= 2
                QueenAnt.doubled_ants.extend([ant])
        ...
        if self.is_impostor == True:
            ...
            scope = self.place.exit
            while scope:
                if scope.ant:
                    double_damage(scope.ant)
                    if hasattr(scope.ant,'contained_ant') and scope.ant.contained_ant:
                            double_damage(scope.ant.contained_ant)
                scope = scope.exit
...
def apply_effect(effect, bee, duration):
    ...
    def effected_action(colony):
        nonlocal effect,bee,duration,original,count
        if count == duration:
